...
**2. QR code for physical event check-in** — an organizer-displayed QR code (printed or shown on a screen at the venue) that a **logged-in** attendee scans with their own phone to check themselves in, hitting the existing `TournamentParticipant.checkedIn` flow (Phase 5 Part C's organizer-console check-in, currently organizer-only — this adds a **self-service** path). The QR encodes a URL like `/events/[id]/checkin?t=<token>` where `<token>` is a short, event-scoped, server-issued value (not the raw tournament id alone — printing/displaying the bare id would let anyone check in remotely, e.g. from a photo shared online). **Documented limitation, explicit**: this is a convenience/self-service mechanism, not an anti-fraud system — a photo of the printed QR shared outside the venue could still be scanned by a logged-in remote user, since the platform has no geofencing or physical-presence proof. If this matters for a specific event, the organizer's existing manual check-in in the organizer console remains the authoritative fallback. Do not scope-creep into building geolocation verification for this phase.

**3. QR code per table for match check-in** — a QR code physically placed at a numbered match table; a player scans it (logged in) to signal "I'm at table N, present for my assigned match," notifying the assigned judge (reuse the Phase 3 notification pipeline — `lib/notify.ts`'s per-user Redis pub/sub channel pattern — rather than inventing a new delivery mechanism). Requires a new field, additive: `Match.tableNumber Int?` (organizer assigns via the organizer console when generating/running a stage — a simple per-match input, not auto-assigned, since physical table layout is the organizer's call) and `Match.player1TableCheckedInAt`/`player2TableCheckedInAt DateTime?`. The QR encodes `/tournaments/[id]/matches/[matchId]/table-checkin?table=N` (the table number in the URL is a redundancy check, not a security boundary — the route's real authz is "the scanning session's `user.id` must be `Match.player1Id` or `Match.player2Id`," otherwise 403, same standing negative-authz-test rule as every other state-changing route). Judge-facing: `JudgeScorePad`/`OrganizerConsole` shows a "beide Spieler anwesend" indicator once both timestamps are set.
