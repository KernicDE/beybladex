...
**Tracking:** [Issue #13](https://github.com/KernicDE/beybladex/issues/13) (closed)

**Status: DONE — merged to main 2026-09-10 (commit `49bfabf`).** Web Push infra (VAPID, `PushSubscription`, subscribe/unsubscribe routes, `sw.js` listeners, real Docker build-arg wiring), the three lifecycle triggers, granular match-lifecycle settings. Reviewed by Kimi before merge — traced all four notification call sites line-by-line and found three genuinely missed bracket-progression paths (no-show route, double-bye generation, grand-final reset) plus an arena-timing gap; all fixed before merge (commit `d2154ad`), including a corrected minor-ceiling justification. `npx tsc --noEmit` clean, eslint clean, 259/259 unit tests. **This was MVP2's 7th and final originally-planned phase** — Phase 21 (avatars, added mid-MVP2) is the remaining piece before MVP2 is fully done.
