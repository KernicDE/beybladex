...
# Phase 10: Profile/Tournament/Rules Depth & Missing UI Surfaces — added post-Phase-6 by explicit user request, NOT YET SCHEDULED
...
**A genuinely bigger gap than it first looked, verified this session**: there is currently **no way to create a `Build` at all, anywhere in the app.** `grep`ing the entire codebase for `prisma.build.create` outside test fixtures and `prisma/seed.mjs` returns nothing — `app/api/builds/route.ts` only exports `GET` (search). The deck builder (`components/beyblade/DeckBuilder.tsx`) only ever *searches* existing builds; nothing ever makes a new one. Every `Build` row that exists today came from the seed script. This phase is not a small addition on top of a working "create a build" flow — that flow needs to be built from scratch, and the user's request (name real retail products like "Reaper Rhino C 4-55D," attach images, link to ownership) shapes what it should look like from the start rather than bolting these on afterward.

**1. Official Sets: a named retail product wrapping a Blade+Ratchet+Bit combo.** Extend `Build` (additive): `name String?` (the retail product name, e.g. "Reaper Rhino C 4-55D" — nullable, since a community/organizer-created custom combo mixing parts from different retail sets has no single official name), `isOfficialSet Boolean @default(false)` (distinguishes TRUSTED/ADMIN-curated official sets from ad-hoc combos), `imageUrl String?` (box art — see item 3). `POST /api/admin/builds` (new route, TRUSTED/ADMIN-only, same authz tier as `/api/admin/parts`): accepts a set name plus three part references, where each of blade/ratchet/bit can be **either an existing `Part.id` (search-and-select) or a brand-new part's fields (inline-create)** — the whole thing (up to 3 new `Part` rows + 1 `Build` row) is one transaction, so the admin never has to leave the page and make three separate trips through `/settings/admin/parts` first the way they currently must. The existing single-part "Neues Teil" flow in `PartForm.tsx` stays as-is for adding a part to an *existing* set/build later, or a standalone accessory part — this is a new, additional flow, not a replacement.

**2. User-created (non-official) builds stay possible too.** Once `POST /api/builds` (a second, non-admin-gated route, or the same route with `isOfficialSet` forced `false` for non-TRUSTED callers — decide at implementation time) exists, any logged-in user can register a custom combo they've assembled from parts across different sets — this is what the deck builder's "no matching build found, create one" path should offer instead of a dead end, closing the gap the original Phase 5 Part A review flagged (`app/builds/page.tsx` could browse builds but nothing could add to that catalog).

**3. Part/Set image upload.** Phase 5 Part A deliberately scoped catalog images as "admin/TRUSTED-curated, served from `/public/parts/`, no upload pipeline needed at this scale" — the user now wants a real upload UI, which needs an actual pipeline, not just a URL text field. **Binding operational note**: this repo's Docker container filesystem is ephemeral (Watchtower recreates the container on every deploy, wiping anything written inside the image's own filesystem) — uploaded images MUST land on a persistent, bind-mounted volume declared in `compose.yml` (a new `parts_uploads:` volume, analogous to the existing `postgres_data:` one), never inside the container's writable layer. `POST /api/admin/parts/[id]/image` (and the equivalent for `Build`/Sets from item 1): multipart upload, TRUSTED/ADMIN-only, validated file type (image only) and size cap, stored to the mounted volume and served via a Next.js route or static file serving pointed at that volume — NOT re-introducing a third-party image CDN/host, consistent with the zero-external-CDN policy. Resize/thumbnail generation (so a grid of collection items never ships an unoptimized multi-MB original — the original Part A concern) can reuse `next/image`'s existing on-the-fly optimization if the volume is reachable by the Next.js process, or a small `sharp`-based resize step on upload if not — decide at implementation time based on how the volume ends up mounted.

**4. Better discoverability.** `lib/buildSearch.ts`/`app/builds/page.tsx` currently support a single free-text query. Add real filters (category, manufacturer, `BeyType`, official-sets-only vs. all), and make the entry point more prominent than today's easy-to-miss `/builds` route buried in navigation — cross-link from `/decks` (the deck builder's part-picker), `/collection` (item 5), and consider a `/builds` entry in `Header`'s primary nav if it isn't already there (verify at implementation time).

**5. Collection-linked availability — the key integration point, added by explicit user follow-up ("der User soll ein Set als gekauft markieren können und dann die Teile für die Builds zur Verfügung haben").** Buying a physical retail Set gives the owner three physical parts at once; the collection and deck-building flows should reflect that:
   - **"Set als gekauft markieren"**: a new collection-entry path alongside the existing single-Part `CollectionItemForm` flow — selecting an official `Build` (item 1) and marking it purchased creates **three linked `CollectionItem` rows** (one per constituent Part — blade, ratchet, bit — preserving the existing "a `CollectionItem` tracks one physical part" design, since parts get separated, worn, or resold individually even if bought together), sharing the entered `purchasePrice`/`currency`/`merchant`/`boughtAt`. Add `CollectionItem.sourceBuildId String?` (nullable FK to `Build`, additive) purely as provenance — "this part entry came from buying Set X" — not a new ownership unit; the three rows remain independently editable/deletable afterward (e.g. if the user later sells just the bit).
...
**Tests:** `tests/integration/admin-create-build.test.ts` (the inline new-parts-plus-build transaction, TRUSTED/ADMIN-only, negative-authz test), `tests/integration/mark-set-purchased.test.ts` (marking a Set purchased creates exactly three `CollectionItem` rows sharing `sourceBuildId`, each independently editable afterward), `tests/unit/build-availability.test.ts` (a build is "available" only when the user owns all three constituent parts — partial ownership of 2 of 3 does not count), `tests/integration/part-image-upload.test.ts` (file-type/size validation, TRUSTED/ADMIN-only, the uploaded file is reachable at its served URL after upload — this one specifically needs the persistent-volume operational piece in place to mean anything in CI, flag if CI's environment can't exercise a real persistent volume and a lighter-weight proof is needed there).

**When this phase is eventually scheduled**, write its own bite-sized TDD sub-plan before starting — this section is file/interface-level, not implementation-ready. Note this phase's own internal dependency: item 5 needs item 1 (official Sets with names) to exist first — a "mark as purchased" flow over anonymous, unnamed combos would be confusing to use.
