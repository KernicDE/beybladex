...
//   POST — body { name, format, order?, swissRounds?, qualifyCount? }: creates a TournamentStage.
//     `order` defaults to the next 1-based sequence slot; SWISS requires swissRounds ≥ 1.
...
const FORMATS = new Set(['SINGLE_ELIMINATION', 'DOUBLE_ELIMINATION', 'SWISS'])
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const stage = await prisma.tournamentStage.create({
    data: { tournamentId: id, order, name, format: format as 'SINGLE_ELIMINATION' | 'DOUBLE_ELIMINATION' | 'SWISS', swissRounds, qualifyCount },
  })
